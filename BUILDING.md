...
Get-Item .\dist\SpeechRibbon-0.0.9.exe
```
...
dotnet publish .\src\SpeechRibbon\SpeechRibbon.csproj --no-build -c Release -r win-x64 --self-contained true -p:BundleThirdParty=false -o .\build\payload-0.0.9-external
New-Item -ItemType Directory -Path .\artifacts -Force | Out-Null
.\tools\launcher\Build-SpeechRibbonLauncher.ps1 -PayloadPath .\build\payload-0.0.9-external\SpeechRibbon.exe -OutputPath .\artifacts\SpeechRibbon-0.0.9.exe
```
...
```powershell
$file = Get-Item .\artifacts\SpeechRibbon-0.0.9.exe
...
Собранный локально EXE должен иметь версию `0.0.9.0` и запускаться на Windows x64. Он может отличаться по контрольной сумме из-за версий инструментов и метаданных сборки. Указанный в README SHA-256 относится к файлу `dist\SpeechRibbon-0.0.9.exe`.
